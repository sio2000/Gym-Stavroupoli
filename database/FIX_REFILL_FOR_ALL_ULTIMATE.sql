-- ═══════════════════════════════════════════════════════════════════════════════
-- ΔΙΟΡΘΩΣΗ: Refill για ΟΛΟΥΣ τους Ultimate χρήστες
-- ═══════════════════════════════════════════════════════════════════════════════
-- Πρόβλημα: Η function ψάχνει μόνο για source_package_name = 'Ultimate'
-- αλλά κάποια memberships δεν έχουν αυτό συμπληρωμένο!
-- Λύση: Ψάχνουμε επίσης με βάση το package_id (membership_packages.name)
-- ═══════════════════════════════════════════════════════════════════════════════

-- ΒΗΜΑ 1: Δες ΟΛΟΥΣ τους Ultimate χρήστες (με ή χωρίς source_package_name)
SELECT 'ΒΗΜΑ 1: Όλοι οι Ultimate χρήστες' as step;
...
SELECT 
    ...
    CASE 
        WHEN m.source_package_name IS NULL THEN '❌ ΛΕΙΠΕΙ source_package_name'
        ELSE '✅ OK'
    END as status
...
ORDER BY m.source_package_name NULLS FIRST, up.last_name;

-- ΒΗΜΑ 2: Διόρθωσε τα memberships που δεν έχουν source_package_name
SELECT 'ΒΗΜΑ 2: Διόρθωση source_package_name' as step;

UPDATE public.memberships m
SET source_package_name = mp.name
FROM public.membership_packages mp
WHERE m.package_id = mp.id
AND mp.name IN ('Ultimate', 'Ultimate Medium')
AND m.is_active = true
AND m.source_package_name IS NULL;

-- ΒΗΜΑ 3: Ενημέρωση function για να ψάχνει και με package_id
SELECT 'ΒΗΜΑ 3: Ενημέρωση function' as step;
...
BEGIN
    ...
    -- *** ΔΙΟΡΘΩΣΗ: Ψάχνουμε με ΠΟΛΛΑΠΛΟΥΣ τρόπους ***
    -- 1. source_package_name IN ('Ultimate', 'Ultimate Medium')
    -- 2. Ή package_id που αντιστοιχεί σε Ultimate/Ultimate Medium
    FOR v_user_record IN (
        SELECT DISTINCT ON (m.user_id)
            ...
            m.end_date,
            -- Παίρνουμε το package name είτε από source_package_name είτε από τον πίνακα packages
...
-- ΒΗΜΑ 4: Διαγραφή σημερινών refills και εκτέλεση
SELECT 'ΒΗΜΑ 4: Εκτέλεση refill' as step;
...
-- ΒΗΜΑ 5: Επαλήθευση
SELECT 'ΒΗΜΑ 5: Επαλήθευση' as step;
...
SELECT 
    ...
    pd.deposit_remaining,
    CASE 
        WHEN mp.name = 'Ultimate' AND pd.deposit_remaining = 3 THEN '✅ ΣΩΣΤΟ'
        WHEN mp.name = 'Ultimate Medium' AND pd.deposit_remaining = 1 THEN '✅ ΣΩΣΤΟ'
        ELSE '❌ ΛΑΘΟΣ'
    END as status
FROM public.memberships m
JOIN public.user_profiles up ON m.user_id = up.user_id
