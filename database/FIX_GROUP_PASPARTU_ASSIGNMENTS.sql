-- Fix Group Paspartu Assignments
...
-- Insert group assignments directly (bypassing RLS)
...
-- =============================================
-- 2. UPDATE SCHEDULE WITH SESSIONS FROM ASSIGNMENTS
-- =============================================

SELECT 'Updating schedule with sessions...' as info;

-- Update the schedule_data.sessions with the group assignments
    ...
    schedule_data = jsonb_set(
        jsonb_set(
            ...
            (
                SELECT jsonb_agg(
                    jsonb_build_object(
                        'id', 'group-session-' || row_number() OVER (ORDER BY ga.start_time),
                        'date', '2025-09-25', -- Default date
                        'startTime', ga.start_time,
                        'endTime', ga.end_time,
                        'type', 'personal',
                        'trainer', ga.trainer,
                        'room', ga.room,
                        'notes', 'Group Paspartu Session ' || row_number() OVER (ORDER BY ga.start_time)
                    ) ORDER BY ga.start_time
                )
                FROM group_assignments ga
                WHERE ga.user_id = '7998415e-2b0b-4a40-9824-cadbd80f6f48'
                AND ga.program_id = '962da64f-3c21-4562-ae8f-c28674d676dc'
            )
        ),
        '{notes}',
        '"Group Paspartu program with sessions from group assignments"'
    ),
    monthly_total = (
        SELECT COUNT(*)
        FROM group_assignments ga
