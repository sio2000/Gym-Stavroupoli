-- Replace All Workout Programs Data
-- This script deletes all existing workout programs data and inserts the correct data with Greek descriptions and YouTube URLs
-- Run this script to ensure clean data

-- First, delete all existing data (in reverse order of dependencies)
DELETE FROM combined_program_exercises;
DELETE FROM exercise_set_configs;
DELETE FROM workout_exercises;
DELETE FROM combined_workout_programs;
DELETE FROM workout_program_categories;

-- Insert Categories
INSERT INTO workout_program_categories (name, name_english, icon, description, display_order) VALUES
  ('Leg & Glutes (πόδια & γλουτός)', 'Leg & Glutes', '🦵', 'Ασκήσεις για πόδια και γλουτούς', 1),
  ('Abdominals (ABS) (κοιλιακοί)', 'Abdominals', '💪', 'Ασκήσεις για τους κοιλιακούς μυς', 2),
  ('Arms & Traps (χέρια & τραπεζοειδής)', 'Arms & Traps', '💪', 'Ασκήσεις για χέρια και τραπεζοειδή μυ', 3),
  ('WARM UP', 'Warm Up', '🔥', 'Ασκήσεις προθέρμανσης', 4),
  ('COOL DOWN', 'Cool Down', '🧘', 'Ασκήσεις ψύξης και τέντωματος', 5),
  ('Lower Back (Ραχιαίοι)', 'Lower Back', '🦴', 'Ασκήσεις για τους ραχιαίους μυς', 6);

-- Get category IDs and insert exercises
DO $$
DECLARE
  leg_glutes_id UUID;
  abdominals_id UUID;
...
BEGIN
  ...
  -- Insert LEG & GLUTES Exercises with Greek descriptions and YouTube URLs
  ...
  -- Insert ABDOMINALS Exercises with Greek descriptions and YouTube URLs
  INSERT INTO workout_exercises (name, name_english, description, youtube_url, category_id, display_order) VALUES
  ...
  -- Insert ARMS & TRAPS Exercises with Greek descriptions and YouTube URLs
  -- Biceps
  INSERT INTO workout_exercises (name, name_english, description, youtube_url, category_id, display_order) VALUES
  ...
  -- Insert WARM UP Exercises with Greek descriptions and YouTube URLs
  INSERT INTO workout_exercises (name, name_english, description, youtube_url, category_id, display_order) VALUES
  ...
  -- Insert COOL DOWN Exercises with Greek descriptions and YouTube URLs
  INSERT INTO workout_exercises (name, name_english, description, youtube_url, category_id, display_order) VALUES
  ...
  -- Insert LOWER BACK Exercises with Greek descriptions and YouTube URLs
  INSERT INTO workout_exercises (name, name_english, description, youtube_url, category_id, display_order) VALUES
...
-- Insert Combined Programs (ONLY if they don't exist - admins will add exercises through the panel)
