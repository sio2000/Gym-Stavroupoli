-- CREATE ULTIMATE LOCKING SYSTEM - Δημιουργία ξεχωριστού συστήματος για Ultimate
...
-- Μετεγκατάσταση υπαρχόντων Ultimate κλειδωμάτων
-- (Χωρίς locked_by γιατί δεν υπάρχουν αυτές οι στήλες)
INSERT INTO ultimate_installment_locks (
    membership_request_id,
    installment_number,
    locked_by,
...
SELECT 
    mr.id,
    1,
    NULL, -- Δεν υπάρχει installment_1_locked_by
    NULL,
    NULL
...
SELECT 
    mr.id,
    2,
    NULL, -- Δεν υπάρχει installment_2_locked_by
    NULL,
    NULL
FROM membership_requests mr
JOIN membership_packages mp ON mr.package_id = mp.id
...
SELECT 
    mr.id,
    3,
    NULL, -- Δεν υπάρχει installment_3_locked_by
