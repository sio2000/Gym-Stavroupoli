...
const GroupTrainingCalendar: React.FC<GroupTrainingCalendarProps> = ({ 
  featureEnabled = true 
}) => {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      console.error('[GroupTrainingCalendar] Error loading events:', error);
      toast.error('Αποτυχία φόρτωσης ημερολογίου');
    } finally {
      setLoading(false);
    }
  }, [currentYear, currentMonth, featureEnabled]);
  ...
  const handleEventClick = async (event: GroupTrainingCalendarEvent) => {
    try {
      ...
      // For merged events, we need to get the first session ID from the event
      // The event.id format is: group-${eventKey} where eventKey is like "2025-09-14-18:00:00-19:00:00-Mike-Room A"
      // We need to find the actual session ID for this time/room combination
      const eventKey = event.id.replace('group-', '');
      console.log('[GroupTrainingCalendar] Event key:', eventKey);
      
      // For now, let's use the event data directly since it already has all the info we need
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.error('[GroupTrainingCalendar] Error refreshing:', error);
    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-yellow-800">Το Ημερολόγιο Ομαδικής Προπόνησης είναι απενεργοποιημένο</span>
        </div>
      </div>
    );
  }
