/**
 * BASIC TEST RUNNER FOR MEMBERSHIP PACKAGES
 * CommonJS version for simple validation
 */
...
function testFileSystem() {
  ...
  logTest('File System Validation', allFilesExist, {
    ...
    missing_files: requiredFiles.length - existingFiles.length
...
function testPackageJson() {
  ...
  try {
    ...
    const hasTsx = packageJson.devDependencies && packageJson.devDependencies['tsx'];
    
    const isValid = hasTestScripts && hasSupabase && hasTsx;
    
    logTest('Package.json Validation', isValid, {
    ...
    }, !isValid ? 'Missing required scripts or dependencies' : null);
  ...
  apiFiles.forEach(apiFile => {
    try {
      ...
      if (fs.existsSync(apiPath)) {
        ...
        console.log(`   ✅ ${apiFile} - ${content.length} bytes`);
      } else {
  ...
  testFiles.forEach(testFile => {
    try {
      ...
      if (fs.existsSync(testPath)) {
        ...
        console.log(`   ✅ ${testFile} - ${content.length} bytes`);
      } else {
...
function runAllTests() {
  ...
  if (failed === 0) {
    ...
    console.log('✅ You can also run: npm run test:comprehensive (if tsx is working)');
  } else {
